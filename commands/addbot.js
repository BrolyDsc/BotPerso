...
let date = ms(args.slice(3).join(" ").replace("j","d"))
    ...
    db.push(`botmember_${message.guild.id}_${user.id}`,{bot: bot.tag,botid: bot.id, date: date, type:tipe} )
message.react("✅")
if(guild && guild.members.cache.get(user.id))  guild.members.cache.get(user.id).roles.add("903802218816352296").catch()
user.send(`Votre bot: ${bot.tag} est maintenant en ligne Merci pour votre achat !`)
